...
class Firewall extends Controller {
	...
	public function export_strings(): array {
		...
		$strings[] = sprintf(
		/* translators: ... */
			__( 'Login protection %s', 'wpdef' ),
			(bool) ( new Login_Lockout_Model() )->enabled ? __( 'active', 'wpdef' ) : __( 'inactive', 'wpdef' )
		);
		$strings[] = sprintf(
		/* translators: ... */
			__( '404 detection %s', 'wpdef' ),
			(bool) ( new Notfound_Lockout() )->enabled ? __( 'active', 'wpdef' ) : __( 'inactive', 'wpdef' )
		);
		$strings[] = sprintf(
		/* translators: ... */
			__( 'User agent banning %s', 'wpdef' ),
			(bool) ( new User_Agent_Lockout() )->enabled ? __( 'active', 'wpdef' ) : __( 'inactive', 'wpdef' )
		);
	...
	public function config_strings( array $config, bool $is_pro ): array {
		...
		if ( isset( $config['login_protection'] ) ) {
			...
			/* translators: ... */
				__( 'Login protection %s', 'wpdef' ),
				(bool) $config['login_protection'] ? __( 'active', 'wpdef' ) : __( 'inactive', 'wpdef' )
			);
		}
		if ( isset( $config['detect_404'] ) ) {
			...
			/* translators: ... */
				__( '404 detection %s', 'wpdef' ),
				(bool) $config['detect_404'] ? __( 'active', 'wpdef' ) : __( 'inactive', 'wpdef' )
			);
		}
